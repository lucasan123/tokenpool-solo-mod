...
export default class Web3ApiHelper  {
  ...
  */
  ...
  var averageGasPriceWei = gasPriceData.average
    ...
    var finalGasPriceWei = parseInt( averageGasPriceWei )
    ...
    if( typename == 'solution'  )
    {
        ...
        if(!isNaN(gasPriceBoost) && gasPriceBoost > 0){
          finalGasPriceWei += gasPriceBoost
        }


        let maxGasPrice =  parseInt( poolConfig.mintingConfig.maxGasPriceGwei ) 

        return Math.min( finalGasPriceWei , maxGasPrice )
    }
    ...
    if( typename == 'batched_payment'  )
    {
      ...
      let gasPriceBoost =  parseInt( poolConfig.paymentsConfig.gasPriceBoost ) 
      if(!isNaN(gasPriceBoost) && gasPriceBoost > 0){
        finalGasPriceWei += gasPriceBoost
      }
      ...
      return Math.min( finalGasPriceWei , maxGasPrice )
       ...
       // solutionGasPriceWei = Math.min( averageGasPriceWei , finalGasPriceWei )
     
    console.log('WARN: calculating gas fee for unknown type: ', typename)
    return finalGasPriceWei


  } 


  static async getPriorityGasPriceWeiForTxType(typename, poolConfig, mongoInterface)
  {
   

    var priorityGasPriceWei = parseInt( 15 )
    ...
    if( typename == 'solution'  )
    {
        ...
        let configuredPriorityFee =  parseInt( poolConfig.mintingConfig.gasPriorityFeeGwei ) 

        if(!isNaN(configuredPriorityFee)) return configuredPriorityFee
    ...
    { 
      ...
      let configuredPriorityFee =  parseInt( poolConfig.paymentsConfig.gasPriorityFeeGwei ) 
